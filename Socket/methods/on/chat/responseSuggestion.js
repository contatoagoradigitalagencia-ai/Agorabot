...
export async function responseSuggestion(socket, data, callback) {
    ...
    try {
		...
		if (typeof input !== "string") return (callback({ code: 400, error: 'O campo "input" deve ser do tipo string' }));
		...
		if (!res) return (callback({ code: 500, error: "Erro interno do servidor" }));
		callback({ code: 200, suggestion: res });
    } catch (error) {
