...
describe("ON - chats:load_chats", () => {
	...
	test("requisição feita passando um array", async () => {
		...
		expect(res).toEqual({
			code: 400,
			error: '"dateLastChat" é opcional, mas quando informado deve ser um objeto válido no formato { timestamp: string, _id:string }'
		});
	});
	...
	test("requisição feita passando 'dateLastChat.timestamp' como string", async () => {
		...
		expect(secondPage).toEqual({
			code: 422,
			error: '"dateLastChat.timestamp" inválido'
		});
	});
	...
	test("requisição feita passando 'dateLastChat._id' como string", async () => {
		...
		expect(secondPage).toEqual({
			code: 422,
			error: '"dateLastChat._id" inválido'
		});
	});
