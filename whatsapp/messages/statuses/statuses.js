...
*/
export default async function statuses(account, value) {
	for (const status of value.statuses) {
		try {
			if (status.status) await mongodb.saveVisualization(account.idPhone, status.id, status.recipient_id, status.status);	// "sent" "delivered" "read" "failed" "played"
